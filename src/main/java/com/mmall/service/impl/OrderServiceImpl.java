...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse<String> manageSendGoods(Long orderNo){
        ...
        if(order != null){
            if(order.getStatus() == Const.OrderStatusEnum.PAID.getCode()){
                ...
                return ServerResponse.createBySuccess("订单已发货");
            }
        }
        return ServerResponse.createByErrorMessage("订单不存在");
    }
    ...
    private long generateOrderNo(){
        ...
        return currentTime+new Random().nextInt(100);//同一时间处理100订单，模拟高并发   --》正确做法，放一个第二天需求的订单缓存池加守护线程不断往里面扩充
    }
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cartItem : cartList){
            ...
            if(Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()){
                return ServerResponse.createByErrorMessage("产品："+product.getName()+"非在售状态");
            }
            //校验产品库存是否充足
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        String subject = new StringBuilder().append("mmall商城扫码支付订单号:").append(orderNo).toString();
    ...
    public void closeOrder(int hour){
        ...
        for(Order order : orderList){
            ...
            for(OrderItem orderItem : orderItemList){
                //使用Innodb引擎，sql语句中使用主键id查询，防止锁表，for update --悲观锁
